...
export function createApp({ directory = dataDirectory(), legacyFile = path.join(directory,'database.json'), worker = true } = {}) {
  const app = express();
  ...
  app.patch('/api/db',(req,res)=>{
    store.transaction(()=>applyRecordChanges(store,req.user,req.body.changes));
...
export async function start() {
  ...
  if(process.env.JC_WHATSAPP==='1') {
    try {
      ...
      const client=new pkg.Client({authStrategy:new pkg.LocalAuth({dataPath:path.join(dataDirectory(),'whatsapp')}),puppeteer:{headless:true,...(process.env.JC_BROWSER_PATH?{executablePath:process.env.JC_BROWSER_PATH}:{})}});
